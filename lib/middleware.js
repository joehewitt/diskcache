	...
	return function(req, res, next) {
		...
		function sendFromApp(cacheKey, body) {
			if (res.statusCode == 200 && !res.doNotCache) {
				...
				entry.mimeType = res.header('content-type');
		...
		function sendFromCache(entry) {
			...
			if (ifNoneMatch && ifNoneMatch == etag) {
				...
				var body = entry.body;
				if (entry.bodyZipped && canZipType(res) && requestAccepts(req, 'gzip')) {
...
function canZipType(res) {
	var contentType = res.header('content-type');
	return exports.gzipBlacklist.indexOf(contentType) == -1;
}
